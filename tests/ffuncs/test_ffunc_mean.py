...
class TestFfuncMeanMissingness:
    ...
    def test_propagate_missing_return_nan(self, factvar, weights):
        with compare_ccube_to_xcube():
            ...
            # 0           1
            # [2.0, 4.0, 5.0]  [1.0, nan]
            means = ccube([idx1]).mean(factvar, weights)
            ...
            # x|
            # 1|1  [1.0, nan]       []
            means = ccube([idx1, self.idx_with_empty_cell]).mean(factvar, weights)
    ...
    def test_ignore_missing_return_nan(self, factvar, weights):
        with compare_ccube_to_xcube():
            ...
            # 0           1
            # [2.0, 4.0, 5.0]  [1.0, nan]
            means = ccube([idx1]).mean(factvar, weights, ignore_missing=True)
            ...
            # x|
            # 1|1  [1.0, nan]       []
            means = ccube([idx1, self.idx_with_empty_cell]).mean(
    ...
    def test_propagate_missing_return_validity(self, factvar, weights):
        with compare_ccube_to_xcube():
            ...
            # x|
            # 1|1  [1.0, nan]       []
            means, validity = ccube([idx1, self.idx_with_empty_cell]).mean(
                factvar, weights, return_missing_as=(-1, False)
            )
            # Cell (1, 0) MUST be missing, because it had a missing input.
            # Cell (1, 1) MUST be missing, because it had no inputs.
